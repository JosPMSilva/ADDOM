...
- **Language:** Follow the operating system language or choose a shipped locale. This changes translation-backed renderer UI copy, not assistant language or backend behavior. Localization remains partial in `0.1.2-alpha`, so some newer or less common surfaces may still fall back to English. Commands, paths, provider/model IDs, protocol names, and support-critical technical tokens remain canonical English.
