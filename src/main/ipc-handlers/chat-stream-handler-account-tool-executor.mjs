...
export function createOpenAIAccountDynamicToolExecutor({
  ...
  const recordAccountToolWorkflowOutcome = ({
    ...
    recordToolWorkflowOutcome(errorDiagnostics, {
      ...
      canonicalToolName: latestOutcome.canonicalToolName || toolName,
      toolExecutionPath: latestOutcome.toolExecutionPath || '',
  ...
  return async ({ toolName = '', input = {} } = {}) => {
    ...
    if (approval.cancelled || approval.decision !== 'approved') {
      ...
      recordToolStepOutcome({
        ...
        canonicalToolName: executionToolName,
        toolExecutionPath: 'approval',
        cancelled: approval.cancelled === true,
      })
      recordAccountToolWorkflowOutcome({
        toolName: tc.name,
        decision: approval.decision || 'denied',
    ...
    recordToolStepOutcome({
      ...
      inputValidationError: execution.inputValidationError,
      modeCapability: execution.modeCapability,
      canonicalToolName: execution.canonicalToolName,
      toolExecutionPath: execution.toolExecutionPath,
    })
    recordAccountToolWorkflowOutcome({
      toolName: tc.name,
      decision: 'approved',
