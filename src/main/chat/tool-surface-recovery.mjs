...
export function handleHiddenKnownToolRecoveryStep({
  ...
  recordToolStepOutcome({
    ...
    toolExecutionPath: 'capability_recovery',
  })
  const recordedOutcome = turnToolResults[turnToolResults.length - 1] || {}
  recordToolWorkflowOutcome(errorDiagnostics, {
    toolName: tc?.name,
    decision: 'approved',
    isError: true,
    failureClass: recordedOutcome.failureClass || lintResult.failureClass || '',
    failureStage: recordedOutcome.failureStage || '',
    failureReasonCode: recordedOutcome.failureReasonCode || '',
    canonicalToolName: recordedOutcome.canonicalToolName || tc?.name,
    toolExecutionPath: recordedOutcome.toolExecutionPath || 'capability_recovery',
