...
export function formatRuntimeDiagnosticsDetail(diagnostics = {}) {
  ...
  const toolWorkflowFailureStages = summarizeCountMap(source.toolWorkflowFailureStageCounts)
  if (toolWorkflowFailureStages) lines.push(`tool_workflow_failure_stages: ${toolWorkflowFailureStages}`)
  const toolWorkflowFailureReasons = summarizeCountMap(source.toolWorkflowFailureReasonCounts)
  if (toolWorkflowFailureReasons) lines.push(`tool_workflow_failure_reasons: ${toolWorkflowFailureReasons}`)
  const toolWorkflowToolIdentityFailures = summarizeCountMap(source.toolWorkflowToolIdentityFailureCounts)
  if (toolWorkflowToolIdentityFailures) lines.push(`tool_workflow_tool_identity_failures: ${toolWorkflowToolIdentityFailures}`)
  const toolWorkflowExecutionPathFailures = summarizeCountMap(source.toolWorkflowExecutionPathFailureCounts)
  if (toolWorkflowExecutionPathFailures) lines.push(`tool_workflow_execution_path_failures: ${toolWorkflowExecutionPathFailures}`)
