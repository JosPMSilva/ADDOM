...
test('FileTree exposes tree semantics, labeled header actions, and left-edge active highlighting', () => {
  ...
  assert.match(html, /title="C:\/workspace\/addom"/)
  assert.match(html, /aria-label="Project folder: C:\/workspace\/addom"/)
  assert.match(html, /class="min-w-0 shrink truncate">C:\/workspace\/<\/span>/)
  assert.match(html, />C:\/workspace\/<\/span><span[^>]*>addom<\/span>/)
